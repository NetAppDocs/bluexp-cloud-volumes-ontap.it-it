...
Cloud Volumes ONTAP supporta solo le implementazioni ha in zone locali AWS. Di seguito sono riportate le configurazioni di esempio:
...
Cloud Volumes ONTAP supporta solo queste configurazioni. La selezione di qualsiasi altro tipo di disco o istanza EC2 nella configurazione della zona locale di AWS per Cloud Volumes ONTAP potrebbe causare un errore di implementazione. Fare riferimento alla documentazione AWS per i dettagli più recenti e completi di link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["EC2 tipi di istanza nelle zone locali"^].
