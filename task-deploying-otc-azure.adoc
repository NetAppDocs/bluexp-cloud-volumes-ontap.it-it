...
+
A partire da Cloud Volumes ONTAP 9.15.1, è possibile implementare istanze di macchine virtuali in singole zone di disponibilità LRS (Local-Redundant Storage). Questa operazione richiede anche l' `Microsoft.Compute/VMOrchestratorZonalMultiFD`attivazione della funzione per l'abbonamento. Questa funzionalità facilita l'implementazione di coppie ha (High Availability) nella stessa zona di disponibilità. Fare riferimento a link:task-saz-feature.html["Attivare VMOrchestratorZonalMultiFD per singole zone di disponibilità"] e link:task-azure-high-availability-mode.html["Abilitare la modalità ad alta disponibilità in Azure"].
...
+
*** Per le singole zone di disponibilità, selezionare un'area Azure, una zona di disponibilità, VNET e una subnet. A partire da Cloud Volumes ONTAP 9.15.1, è possibile installare istanze VM in una singola zona di disponibilità. È necessario selezionare una regione che supporti questa modalità. Se la regione selezionata dall'utente non supporta la distribuzione zonale, viene seguito il precedente modello di distribuzione non zonale per LRS.
+
Per ulteriori informazioni su questa modalità di distribuzione e sulle zone di Azure supportate, consultare https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Documentazione di Microsoft Azure: Documentazione sui set di scalabilità delle macchine virtuali"^] e https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Sito Web di Microsoft Azure: Prodotti disponibili per area geografica"^].
