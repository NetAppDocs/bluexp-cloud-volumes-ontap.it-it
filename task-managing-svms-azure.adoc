...
NOTE: Per eseguire ulteriori attività relative alle schede NIC, è possibile assegnare un ruolo di contributore della rete o un ruolo personalizzato con le autorizzazioni appropriate in Azure. Per ulteriori informazioni su queste autorizzazioni relative alla scheda NIC, fare riferimento alla https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-network-interface?tabs=azure-portal#permissions["Documentazione di Microsoft Azure"^] .
...
NOTE: Un IP front-end deve essere creato solo sotto NIC0 per ogni SVM corrispondente. Per ulteriori informazioni su come aggiungere l'indirizzo IP alla SVM NIC0, fare riferimento alla "fase 7 [hyperlink]"
...
network route create -vserver <svm-name> -destination 0.0.0.0/0 -gateway <ip-of-gateway-server>
----
. Crea LIF dati. In questa fase, si utilizzano gli IP assegnati in link:task-managing-svms-azure.html#ha-pair["procedura precedente"]per fungere da LIF dati.
+
...
. Crea LIF iSCSI per fornire comunicazioni DNS. Le LIF iSCSI sono necessarie per le comunicazioni DNS anche quando non viene utilizzato il protocollo iSCSI.
+
.. Utilizzare il seguente comando per creare una LIF iSCSI sul nodo 1.
+
[source, cli]
...
network route create -vserver <svm-name> -destination 0.0.0.0/0 -gateway <ip-of-gateway-server>
----
. Crea LIF dati NAS. In questa fase, si utilizzano gli IP assegnati in link:task-managing-svms-azure.html#ha-pair["procedura precedente"]per fungere da LIF dati.
+
.. Utilizzare il seguente comando per creare un LIF NAS sul nodo 1.
+
[source, cli]
