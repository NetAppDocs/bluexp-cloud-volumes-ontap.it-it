...
Se si crea una nuova versione della chiave, Cloud Volumes ONTAP utilizza automaticamente la versione più recente della chiave, garantendo la protezione dei dati senza la necessità di aggiornamenti manuali.

.Fasi
. Accedere al vault delle chiavi nel portale Azure.
. Selezionare il tasto che si desidera ruotare.
. Fare clic su *Nuova versione* per generare una nuova versione della chiave.


Una volta creata una nuova versione, Cloud Volumes ONTAP rileva e utilizza automaticamente l'ultima versione della chiave per la crittografia.
