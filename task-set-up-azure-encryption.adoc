...
Devi configurare le chiavi di crittografia nel portale di Azure. Per informazioni sulla configurazione dei tasti e sull'impostazione della rotazione dei tasti, fare riferimento ai seguenti argomenti della documentazione di Microsoft Azure:
...
NOTE: Dopo aver configurato le chiavi, accertarsi di disporre di https://learn.microsoft.com/en-us/azure/virtual-machines/disk-encryption#customer-managed-keys["Rotazione della chiave abilitata sul portale Azure"^], in modo che Cloud Volumes ONTAP possa utilizzare le nuove chiavi.

Al momento della creazione e dell'abilitazione di una nuova versione delle chiavi, Cloud Volumes ONTAP rileva e utilizza automaticamente la versione più recente per la crittografia, garantendo la protezione dei dati senza la necessità di aggiornamenti manuali.
