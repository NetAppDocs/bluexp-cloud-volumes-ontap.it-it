...
[role="quick-margin-para"]
Il tiering dei dati deve essere attivato su un aggregato per consentire il tiering dei dati su un volume. È necessario conoscere i requisiti per i nuovi volumi e per i volumi esistenti. <<Ensuring that tiering is enabled on aggregates,Scopri di più>>.
...
+
BlueXP crea un archivio di oggetti per i dati cold quando il sistema viene creato, <<Enabling data tiering after implementing the requirements,a condizione che non vi siano problemi di connettività o permessi>>. In seguito, è sufficiente attivare il tiering dei dati sui volumi (e in alcuni casi, <<Ensuring that tiering is enabled on aggregates,sugli aggregati>>).
...
* *Volumi esistenti*
+
Se si desidera attivare il tiering dei dati su un volume esistente, è necessario assicurarsi che il tiering dei dati sia attivato sull'aggregato sottostante. Se il tiering dei dati non è abilitato nell'aggregato esistente, sarà necessario utilizzare Gestione di sistema di ONTAP per collegare un aggregato esistente all'archivio di oggetti.
